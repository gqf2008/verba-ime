...
> 当前推进 **M3 多模态 + M4 TTS Rust 核心**（TTS mock/edge-tts + OCR mock/Windows.Media.Ocr + ASR mock 已端到端通，edge-tts 实机出真实 MP3；Piper/whisper.cpp 跟进）。
...
| 能力 | 说明 |
| --- | --- |
| 🖼️ OCR | 截图选区域 / 剪贴板图片 / 图片文件 → 文字上屏 |
| 🎙️ ASR | 全局快捷键唤起语音输入 → 实时转写 → 上屏 |
| 🤖 LLM（远程） | 输入法内 AI 模式：翻译、润色、续写、总结、自定义 Prompt，流式输出 |
| 🔊 TTS | 朗读选中文本 / 候选词，可配置上屏自动朗读 |
| ⌨️ 输入 | 英文直输 + 标点 + 快捷指令；中文拼音引擎列入路线图（可选集成 librime） |
| ⚙️ 配置 | 统一设置面板（Tauri），AI 服务商可插拔，密钥本地安全存储 |
