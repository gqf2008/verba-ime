...
| M6 | 发布 | 打包、签名、公证、Alpha / Beta、文档与社区运营（v0.2.0–v0.2.12 已发布；v0.2.12：macOS PKG 重定位修复） | M5 |
...
| 2026-09-09 | v0.2.12 已发布（tag 指向 merge commit de67775，GitHub Release run 34323513139 全绿：version-guard + macOS + Windows + release；DMG/PKG/EXE/SHA256SUMS 资产完整，SHA256 与 Release API digest 一致；PKG 使用 Developer ID Installer 签名、公证 trusted、staple 成功，组件与 Distribution 的 relocation 子元素数均为 0、install-location 为 `/Library/Input Methods`；#121 待真机安装/菜单验收）。 |
