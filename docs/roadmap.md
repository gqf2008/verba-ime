...
> Slint 设置面板已落地（apps/settings）；M6 发布已达成（v0.2.0–v0.2.6
> 签名公证产物；v0.2.6 上线：盲按队列化修漏字 / macOS 菜单栏图标 / AI 三态结果浮层 / CapsLock 审查六项，PR #91）；**v0.2.7 发布准备中**（Windows 激活一律中文起步修「默认英文 + `///` 失效」、安装器升级/卸载停旧 daemon + 重启替换，PR #94/#95）。
...
| M6 | 发布 | 打包、签名、公证、Alpha / Beta、文档与社区运营（v0.2.0–v0.2.6 已发布；v0.2.7 发布准备中：Windows 激活默认中文 / 安装器 daemon 生命周期） | M5 |
