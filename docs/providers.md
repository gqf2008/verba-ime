...
- **探针**：`usls` main（`0.2.0-alpha.4`，rev `c15ad77`，`ort = "=2.0.0-rc.13"` **与本仓同版本线，可以共图**）+ `usls::vlm::SmolVLM`，`Config::smolvlm2_256m()`，CPU（macOS aarch64，release）。
...
- **运行开销**（缓存已热）：启动+建会话 1.5–2.7s；单次问答 3.4–10.2s（64–96 tokens，≈10–20 tok/s）；**峰值 RSS 2.0–2.9GB**（1GB 权重 + fp32 激活/arena）。
