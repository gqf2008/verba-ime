...
4. 编译（`/DMyAppVersion` 可选，默认 0.2.6；发布流水线注入根 Cargo.toml 版本）：
   ```powershell
...
打 tag `v*`（如 `git tag v0.2.6 && git push origin v0.2.6`）自动触发；也可 `workflow_dispatch` 干跑（只出 artifact，不发 Release）：
