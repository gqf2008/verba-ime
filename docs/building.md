...
4. 编译（`/DMyAppVersion` 可选，默认 0.2.3；发布流水线注入根 Cargo.toml 版本）：
   ```powershell
   cd frontends/windows/installer
   & "$env:LOCALAPPDATA\Programs\Inno Setup 6\ISCC.exe" /DMyAppVersion=0.2.3 verba-ime.iss
   ```
...
打 tag `v*`（如 `git tag v0.2.3 && git push origin v0.2.3`）自动触发；也可 `workflow_dispatch` 干跑（只出 artifact，不发 Release）：
